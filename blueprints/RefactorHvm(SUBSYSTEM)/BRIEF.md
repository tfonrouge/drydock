...
- **Called by zero files in the entire codebase**

They contain independent type cascades with their own `hb_objHasOperator` /
`hb_objOperatorCall` paths — 680 lines of operator logic that no code path
ever reaches. They were presumably designed for a peephole optimization in
the C code generator that was never implemented (the compiler does emit
`hb_xvmMultByInt` etc. for arithmetic, but never for comparisons).
...
Since they don't, deletion is clean.
