...
- **Called by zero files in source-tree grep** — however Phase 0 was
  implemented then reverted (commit `6f6811b`, 2026-03-27) because these
  functions may be referenced in C code generated by `gencc.c`. The
  "zero callers" analysis only searched source files, not generated output.
  A proper analysis of `gencc.c` emission patterns is needed before
